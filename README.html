<html>
<head><title>The Rolling-Moon</title>
<style type="text/css">
body {
	padding: 2em 28% 3em 12%;
	color: #114;
	background-color: #D8D8EE;
}
a { color: #55C; }
a:hover { background-color: #EBEBFF; }
</style>
</head>
<body><h1>The Rolling-Moon</h1>

	<h2>Dependences</h2>

<p>	To play this game you need the ocaml-chipmunk bindings
	version 0.04.2:<br />
	<a href="http://decapode314.free.fr/ocaml/chipmunk/downloads/ocaml-chipmunk-0.04.2.tar.gz"
		>ocaml-chipmunk-0.04.2.tar.gz</a>
	<br />
	(Don't take a more recent version, the API is not compatible anymore.)
	<br />
	And the graphics rendering is made through the ocaml-opengl bindings
	<a href="http://decapode314.free.fr/ocaml/GL/"
		>glMLite</a>.
</p>
<p>	The path set in "rolling_moon.ml" expect that these 2 dependencies are
	installed in "/tmp", if you put it in another directory, then change
	the path set with "#directory" in "rolling_moon.ml".
</p>
...
<code>	sh mk_level_data.sh  level.x.svg  level.x.data </code>
</p>
<p>	Then just load it in the game with:<br />
<code>	ocaml -w -6 rolling_moon.ml  -l level.x.data </code>
</p>
